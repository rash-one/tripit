<template>
    <div>
      <div class="relative h-screen w-full overflow-hidden">
        ...
        >
          <h1
            class="text-white text-4xl md:text-6xl lg:text-7xl font-bold whitespace-nowrap"
            id="hero-title"
          >
           {{$t('LastingMemories')}}
          </h1>
  ...
  export default {
    ...
    methods: {
      ...
      getAgencyTrips() {
        axios
          .get(
            "https://dqorskomhqezaztrdhzc.supabase.co/rest/v1/trips?select=id,created_at,creator_id,location_id,title,image_name,start_date,end_date,description,users!inner(*),locations!inner(*),trips_images!fk_trips(id,created_at,image_name)&users.role=eq.tour",
            {
              headers: {
                apikey:
          ...
          .catch((error) => {
            console.error("Error fetching tour trips:", error);
          });
      },
    },
    created() {
