<template>
  <div class="container mx-auto flex justify-center items-center w-full h-full">
    <div
      v-if="loginCheck"
      class="flex w-full max-w-sm overflow-hidden bg-white rounded-lg shadow-md border-2 border-[#ffb830] fixed right-4 top-[10%] z-50 animate-slide-in"
    >
      <div class="flex items-center justify-center w-12 bg-[#ffb830]">
        <svg
      ...
      <div class="px-4 py-2 -mx-3">
        <div class="mx-3 flex flex-col gap-2">
          <span class="font-semibold text-[#ffb830] dark:text-[#ffb830]">
             {{$t('failedBooking')}}
          </span>
      ...
      to="/login"
      @click="goToLogin"
      class="bg-[#ffb830] p-2 rounded-lg text-[#ffffff] font-bold hover:bg-[#feaf55] hover:text-white text-center"
      > {{$t('loginBtn')}}</router-link
    >
        </div>
      </div>
    </div>
    ...
    <div
      class="flex flex-col w-full h-auto mt-[40px] lg:mt-[120px] p-4 lg:p-8 lg:space-x-8 space-x-0 lg:flex-row pt-20 gap-4 lg:gap-0"
    >
      <!-- Booking Form -->
      <div
        class="w-full p-8 space-y-0 lg:space-y-4 bg-[#ffe7bb] shadow-md rounded-2xl flex flex-col mt-10 lg:mt-0 mb-10 lg:mb-0 overflow-y-auto [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:rounded-2xl [&::-webkit-scrollbar-thumb]:rounded-2xl [&::-webkit-scrollbar-thumb]:bg-[#ffb830]"
      >
        <div
          class="flex flex-col xl:flex-row w-full xl:max-h-[400px] gap-2 lg:gap-4 mb-2"
        >
          <img
            :src="getImageUrl(trip.image_name)"
            alt=""
            class="rounded-2xl object-cover object-center w-full xl:w-8/12"
          />
          <div class="flex flex-col gap-4 py-2">
            <div class="title gap-4 flex flex-col">
              <h1 class="text-3xl font-bold text-[#ffb830]">
                {{ trip.title }}
              </h1>
            ...
            <div>
              <div class="tripIns w-full h-full flex flex-col gap-4">
                <div class="location flex flex-row gap-1 items-center">
                  ...
                  <p class="text-md font-semibold text-center"> {{$t('location')}} :</p>
                  <p class="text-md text-center">
                    {{ trip.locations.gov_name }}
                  </p>
                </div>
                <div class="duration flex flex-row gap-1 items-center">
                  ...
                  <p class="text-md font-semibold text-center"> {{$t('Duration')}} :</p>
                  <p class="text-md text-center"> {{$t('Duration2')}}</p>
                </div>
                <div class="language flex flex-row gap-1 items-center">
                  ...
                  <p class="text-md font-semibold text-center">{{$t('language')}}:</p>
                  <p class="text-md text-center">{{$t('language2')}}</p>
                </div>
                <div class="provider flex flex-row gap-1 items-center">
                  ...
                  <p class="text-md font-semibold text-center">{{$t('Provider')}}:</p>
                  <p class="text-md text-center">{{ trip.users.username }}</p>
                </div>
              </div>
            </div>
          </div>
        </div>
        <hr class="border-2 border-[#ffb830] rounded-lg w-full mx-auto mt-1" />
        <div>
          <label class="block text-lg font-medium text-[#ffb830]"
            > {{$t('SelectDate')}}</label
          >
          <input
            type="date"
            ...
            :min="minDate"
            :max="maxDate"
            class="mt-0 lg:mt-1 block w-full p-2 border rounded focus:outline-none focus:ring-2 focus:ring-[#Ffb830]"
        ...
        <div class="flex flex-col gap-2">
          ...
          <input
            ...
            min="1"
            max="10"
            class="block w-full p-2 border rounded focus:outline-none focus:ring-2 focus:ring-[#Ffb830]"
          />
          <label class="block text-lg font-medium text-[#ffb830]"
            > {{$t('Childern1')}}:</label
          >
        ...
        <div class="mt-8 flex flex-row justify-between">
          <p class="text-xl font-bold text-[#333333]">{{$t('totalPrice')}}:</p>
          <p class="text-xl font-bold text-[#ffb830]">{{ totalPrice }} EGP</p>
        </div>
        <!-- Button -->
        <div>
          <button
            @click="checkout"
            class="bg-[#ffb830] p-3 rounded-lg text-[#ffffff] font-bold hover:bg-[#feaf55] hover:text-white w-full"
          >
             {{$t('confirmPayment')}}
          </button>
        </div>
...
export default {
  ...
  data() {
    this.pk =
      "pk_test_51Q17wy02jj3zI6IguSCfmbyrKzdm6L6ewdaJStSp5WxxN2ru20yweZxhLJQJDrGElT4ruO9zSSFIMik0GBjyarQl00PyUDiVUl";
    return {
      trip: null,
      loginCheck: false,
      selectedDate: null,
      ...
      lineItems: [
        {
          price: "price_1Q1CI302jj3zI6IgGtbtEMFK",
          quantity: 1,
        },
      ],
      successUrl: "http://localhost:5173/checkoutdone",
      cancelUrl: "http://localhost:5173/checkoutFailed",
    };
  },
  computed: {
    minDate() {
      return this.trip
        ? new Date(this.trip.start_date).toISOString().split("T")[0]
        : "";
    },
    maxDate() {
      return this.trip
        ? new Date(this.trip.end_date).toISOString().split("T")[0]
        : "";
    },
    totalPrice() {
      const adultPrice = this.trip ? this.trip.price * this.numAdults : 0;
      const childrenPrice = this.trip
        ? this.trip.price * 0.75 * this.numChildren
        : 0;
      const total = adultPrice + childrenPrice;
      return total;
    },
  },
  methods: {
    ...
    gettripid() {
      ...
      axios
        .get(
          `https://dqorskomhqezaztrdhzc.supabase.co/rest/v1/trips?id=eq.${tripId}&select=id,created_at,creator_id,location_id,title,start_date,image_name,price,end_date,description,users!inner(*),locations!inner(*),trips_images!fk_trips(id,created_at,image_name)`,
          {
            headers: {
              apikey:
    ...
    checkout() {
      ...
      const totalTravelers =
        this.numAdults + this.numChildren + this.numChildrenUnder6;

      // Calculate the total price (already handled in computed property)
      ...
      axios
        .post(
          "https://dqorskomhqezaztrdhzc.supabase.co/rest/v1/booked_trips",
          bookingData,
          {
            headers: {
              apikey:
